    ...
    jsonp: function (obj) {
      ...
      heatmap.$zd._.jsonp({
        ...
        success: function (data) {
          ...
          if (data && heatmap.$zd._.isObject(data) && data.success) {
            ...
            if (data && heatmap.$zd._.isObject(data) && data.success === false) {
              error({
        ...
        error: function (err) {
          ...
          if (err === 'timeout') {
            ...
            if (heatmap.$zd._.URL(location.href).protocol === 'https:' && heatmap.$zd._.URL(obj.url.jsonp).protocol === 'http:') {
              ...
              error({
                error: '<div>后台 JSONP 地址不通，可能原因如下：</div><div style="font-size:15pxmargin-top:10pxtext-align:leftdisplay:inline-block"><div>1. 检查神策分析版本是否低于 v2.2.0，如果低于此版本请联系值班同学升级版本</div><div style="margin-top:5px">2. 神策分析后台不允许访问，请贵司研发参考当前页面的控制台报错，去对应的后台配置解决</div></div>'
              })
            }
          }
        }
